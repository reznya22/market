...
class Book(InfoMixin):
    ...
    publication_year = models.PositiveIntegerField(
        verbose_name='Год публикации', blank=True)
    genre = models.ForeignKey(to=Genre, on_delete=models.CASCADE)
    ISBN = models.PositiveBigIntegerField(
        verbose_name='ISBN', unique=True, blank=True)
    price = models.DecimalField(
        verbose_name='Цена', max_digits=5, decimal_places=2, null=True)
    image = models.ImageField(verbose_name='Обложка', upload_to='%Y/%m/%d')
    description = models.TextField(
        verbose_name='Описание', max_length=500, blank=True)
    quantity_on_stock = models.PositiveIntegerField(
        verbose_name='Кол-во на складе', default=0)
...
class Review(models.Model):
    ...
    review_text = models.TextField(
        verbose_name='Текст отзыва', max_length=500)
